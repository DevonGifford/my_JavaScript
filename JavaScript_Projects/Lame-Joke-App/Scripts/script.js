...
function tellMe(joke) {
    ...
    VoiceRSS.speech({
        key: apiKey,    //INSERT YOUR API KEY HERE
        src: joke,
        hl: 'en-us',
        v: 'Linda',
        r: 0, 
...
// => disable/enable the button until the joke is finished
...
async function getJokes() {
    ...
    try{
        ...
        // => handle the possiblity of a ['setup' & 'punchline'] type of joke
        data.setup
        ? joke = `${data.setup} ... ${data.delivery}`
        : joke = data.joke

        // => pass the joke to TTS api
        tellMe(joke);

        // => disable the joke button until joke is complete
        toggleButton()
